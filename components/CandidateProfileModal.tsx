...
const CandidateProfileModal: React.FC<CandidateProfileModalProps> = ({ candidate, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 bg-opacity-70 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="bg-surface dark:bg-dark-surface rounded-xl shadow-2xl w-full max-w-lg transform transition-all animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row items-center sm:items-start sm:space-x-6 mb-6">
                ...
                <div className="text-center sm:text-left mt-4 sm:mt-0">
                    <h2 className="text-3xl font-bold text-primary dark:text-dark-primary">{candidate.name}</h2>
                    <p className="text-text-secondary dark:text-slate-300 mt-2">{candidate.bio || 'Bio singkat kandidat.'}</p>
                </div>
            </div>
            <div className="space-y-4 text-sm">
                <div>
                    <h3 className="font-bold text-primary dark:text-dark-primary tracking-wide uppercase">Visi</h3>
                    <p className="text-text-secondary dark:text-slate-300 mt-1">{candidate.vision}</p>
                </div>
                 <div>
                    <h3 className="font-bold text-primary dark:text-dark-primary tracking-wide uppercase">Misi</h3>
                    <p className="text-text-secondary dark:text-slate-300 mt-1 whitespace-pre-line">{candidate.mission}</p>
                </div>
            </div>
        </div>
