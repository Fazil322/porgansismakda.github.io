    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-primary dark:text-dark-primary">Manajemen Direktori Organisasi</h3>
                ...
                <table className="w-full text-sm text-left text-text-secondary dark:text-dark-text-secondary">
                    ...
                    <tbody>
                        {organizations.map(org => (
                            <tr key={org.id} className="bg-surface dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                <td className="px-6 py-4">
                                    <img src={org.imageUrl} alt={org.name} className="h-10 w-10 rounded-full object-cover"/>
                                </td>
                                <td className="px-6 py-4 font-medium text-primary dark:text-dark-primary">{org.name}</td>
                                <td className="px-6 py-4 text-text-primary dark:text-dark-text-primary">{org.members.length}</td>
                                <td className="px-6 py-4 text-right space-x-4">
                                    <button onClick={() => openModalForEdit(org)} className="font-medium text-accent hover:text-accent-hover dark:text-cyan-400 dark:hover:text-cyan-300 transition-colors">Edit</button>
                                    <button onClick={() => setDeletingOrg(org)} className="font-medium text-red-600 hover:text-red-700 dark:text-red-500 dark:hover:text-red-400 transition-colors">Hapus</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
