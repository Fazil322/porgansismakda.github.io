    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-primary">Manajemen Agenda Kegiatan</h3>
                ...
                <table className="w-full text-sm text-left text-secondary-dark">
                    ...
                    <tbody>
                        {activities.map(activity => (
                            <tr key={activity.id} className="bg-surface border-b border-slate-200 hover:bg-slate-50">
                                <td className="px-6 py-4 font-medium text-primary">{activity.date}</td>
                                <td className="px-6 py-4">{activity.title}</td>
                                <td className="px-6 py-4">{activity.organizer}</td>
                                <td className="px-6 py-4 text-right space-x-4">
                                    <button onClick={() => openModalForEdit(activity)} className="font-medium text-accent hover:text-accent-hover transition-colors">Edit</button>
                                    <button onClick={() => setDeletingActivity(activity)} className="font-medium text-red-600 hover:text-red-700 transition-colors">Hapus</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
