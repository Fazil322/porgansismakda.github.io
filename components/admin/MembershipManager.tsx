...
const MembershipManager: React.FC<MembershipManagerProps> = ({ registrations, onUpdateStatus, addToast }) => {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold text-primary mb-6">Manajemen Pendaftaran Anggota</h3>
                ...
                <table className="w-full text-sm text-left text-secondary-dark">
                    ...
                    <tbody>
                        {registrations.length > 0 ? registrations.map(reg => (
                            <tr key={reg.id} className="bg-surface border-b border-slate-200 hover:bg-slate-50">
                                <td className="px-6 py-4 font-medium text-primary">{reg.studentName}</td>
                                <td className="px-6 py-4">{reg.studentClass}</td>
                                <td className="px-6 py-4">{reg.organizationName}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${statusBadge(reg.status)}`}>
                                        {reg.status}
                                    </span>
                                </td>
                                <td className="px-6 py-4 text-right space-x-2">
                                    {reg.status === 'pending' ? (
                                        ...
                                        <span className="text-slate-400">Ditindaklanjuti</span>
                                    )}
                                </td>
                            </tr>
                        )) : (
                            <tr>
                                <td colSpan={5} className="text-center p-8 text-secondary">Belum ada pendaftaran anggota.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {action && (
                ...
                >
                    ...
                    {action.reg.reason && <div className="mt-4"><p className="font-semibold">Alasan pendaftar:</p><p className="text-sm p-2 bg-slate-100 rounded border mt-1">{action.reg.reason}</p></div>}
                </Modal>
            )}
        </div>
    );
